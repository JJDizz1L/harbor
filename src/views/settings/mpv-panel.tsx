...
import { ColorHdrSection, PictureDialsSection } from "./mpv-panel/dials";
      ...
      >
        <Segmented
          ...
          options={[
            ...
            { value: "cuda-copy", label: "CUDA" },
            { value: "cuda", label: "CUDA interop" },
        ...
        <p className="mt-2.5 text-[12.5px] leading-relaxed text-ink-subtle">
          {settings.mpvHwdec === "off"
            ...
            : settings.mpvHwdec === "cuda-copy"
              ? t(
                  "Uses NVIDIA's NVDEC decoder and keeps decoded frames in system memory. Most compatible on Wayland and works across all display servers.",
                )
              : settings.mpvHwdec === "cuda"
                ? t(
                    "Uses NVIDIA's NVDEC decoder with GPU-memory interop. Can be faster on X11 but may glitch on Wayland. Try CUDA (copy) instead if you see artifacts.",
                  )
